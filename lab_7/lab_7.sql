...
SELECT ProductID, Name FROM Production.Product WHERE ProductID = 707;
...
CREATE OR ALTER TRIGGER trg_Database_SchemaLog
ON DATABASE
...
BEGIN
    ...
    SELECT EVENTDATA().value('(/EVENT_INSTANCE/EventType)[1]', 'NVARCHAR(100)'),
           ISNULL(EVENTDATA().value('(/EVENT_INSTANCE/ObjectName)[1]', 'NVARCHAR(100)'), 'UNKNOWN'),
           SUSER_SNAME();
END;
GO
